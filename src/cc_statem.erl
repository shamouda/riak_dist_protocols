...
waiting_for_op({call, Sender}, {tx_put, Bucket, Key, Value}, State = #state{tx_id = TxId}) ->
    ...
    {IndexNode, _} = shard_utils:send_to_one_and_return_node(Bucket, Key, {tx_put, TxId, {Key, Value}}),
    {next_state, waiting_for_op, State, {reply, Sender, {ok, IndexNode}}};

waiting_for_op({call, Sender}, {prepare, Nodes}, State = #state{tx_id = TxId}) ->
	?LOG_INFO("current_state[waiting_for_op] action[prepare]", []),
    Response = lists:foldl(fun(Node, OutList) -> 
                    Result = shard_utils:send_to_one(Node, {prepare, TxId}),
                    [Result | OutList]
                end, [], Nodes),
...
waiting_to_commit({call, Sender}, {commit, Nodes}, State = #state{tx_id = TxId}) ->
	?LOG_INFO("current_state[waiting_to_commit] action[commit]", []),
    lists:foreach(fun(Node) -> shard_utils:send_to_one(Node, {commit, TxId}) end, Nodes),
    {next_state, committed, State#state{status = committed}, {reply, Sender, ok}}.

waiting_to_abort({call, Sender}, {abort, Nodes}, State = #state{tx_id = TxId}) ->
	?LOG_INFO("current_state[waiting_to_abort] action[abort]", []),
    lists:foreach(fun(Node) -> shard_utils:send_to_one(Node, {abort, TxId}) end, Nodes),
